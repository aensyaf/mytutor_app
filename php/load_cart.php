<?php
    ...
    $email = $_POST['email'];
    $sqlloadcart = "SELECT tbl_carts.cart_id, tbl_carts.subj_id, tbl_carts.cart_quan, tbl_subjects.subject_name, tbl_subjects.subject_price,
    tbl_subjects.subject_sessions FROM tbl_carts INNER JOIN tbl_subjects ON tbl_carts.subj_id = tbl_subjects.subject_id 
    ...
    if ($result->num_rows > 0) {
        ...
        while ($rows = $result->fetch_assoc()) {
            ...
            $sublist['cart_quan'] = $rows['cart_quan'];
            $sublist['subj_id'] = $rows['subject_id'];
            $price = $rows['cart_quan'] * $subprice;
